...
function CommandSuggestionItem({ suggestion }: { suggestion: CommandSuggestion }) {
  return (
    <div>
      ...
      {suggestion.example && (
        <div className="text-xs text-bolt-elements-textSecondary mt-1 font-mono">
          {suggestion.example}
        </div>
      )}
    </div>
...
function FileSuggestionItem({ suggestion }: { suggestion: { type: 'file' | 'folder', path: string } }) {
  ...
  const displayPath = suggestion.path.length > 50 
    ? '...' + suggestion.path.slice(-47)
    : suggestion.path;
...
function getFileSuggestions(partialPath: string, foldersOnly: boolean = false): Array<{ type: 'file' | 'folder', path: string }> {
