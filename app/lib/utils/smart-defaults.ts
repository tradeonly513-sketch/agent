...
export class SmartDefaults {
   ...
   */
  getSmartDefaults(userApiKeys?: Record<string, string>): { model: string; provider: ProviderInfo } {
    try {
      // Get configured providers
      const configuredProviders = this.llmManager.getConfiguredProviders(userApiKeys);
   ...
   */
  isModelAvailable(modelName: string, userApiKeys?: Record<string, string>): boolean {
    try {
      const configuredProviders = this.llmManager.getConfiguredProviders(userApiKeys);
