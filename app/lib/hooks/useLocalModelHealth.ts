...
export function useLocalModelHealth(options: UseLocalModelHealthOptions = {}): UseLocalModelHealthReturn {
    ...
    const handleStatusChanged = (status: ModelHealthStatus) => {
      setHealthStatuses(current => {
        const index = current.findIndex(
          s => s.provider === status.provider && s.baseUrl === status.baseUrl
        );
  ...
  const startMonitoring = useCallback((provider: 'Ollama' | 'LMStudio' | 'OpenAILike', baseUrl: string, interval?: number) => {
  ...
  const stopMonitoring = useCallback((provider: 'Ollama' | 'LMStudio' | 'OpenAILike', baseUrl: string) => {
    ...
    setHealthStatuses(current =>
      current.filter(s => !(s.provider === provider && s.baseUrl === baseUrl))
    );
  }, []);
  ...
  const isHealthy = useCallback((provider: 'Ollama' | 'LMStudio' | 'OpenAILike', baseUrl: string) => {
