    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.type === 'opaque' || response.status === 0) {
          throw new Error('CORS_ERROR: LM Studio server is not configured to allow requests from this origin. Please configure CORS in LM Studio settings.');
        }
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      ...
      if (errorMessage.includes('CORS') || errorMessage.includes('NetworkError') || errorMessage.includes('Failed to fetch')) {
        return {
          isHealthy: false,
          responseTime: 0,
          error: 'CORS_ERROR: LM Studio server is blocking cross-origin requests. Try enabling CORS in LM Studio settings or use Bolt desktop app.',
        };
      }
